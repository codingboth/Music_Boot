...
async def 나가(ctx):
    try:
        ...
        await ctx.send("이미 그 채널에 접속해있지 않아요.")
...
async def URL재생(ctx, *, url):
    ...
    if not vc.is_playing():
        ...
        await ctx.send(embed = discord.Embed(title= "URL재생", description = "현재 " + url + "을(를) 재생하고 있습니다.", color = 0x00ff00))
    else:
...
async def 재생(ctx, *, msg):
    ...
    if not vc.is_playing():
        ...
        with YoutubeDL(YDL_OPTIONS) as ydl:
            info = ydl.extract_info(url, download=False)
        URL = info['formats'][0]['url']
        await ctx.send(embed = discord.Embed(title= "재생", description = "현재 " + musicnow[0] + "을(를) 재생하고 있습니다.", color = 0x00ff00))
        vc.play(discord.FFmpegPCMAudio(URL, **FFMPEG_OPTIONS), after=lambda e:play_next(ctx))
    else:
        user.append(msg)
